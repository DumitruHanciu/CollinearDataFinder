...
public class Point implements Comparable<Point> {
    ...
    public double slopeTo(Point that) {
        /* YOUR CODE HERE */
        if (that.x == this.x) // -0.0 is default no value for slope
            if (that.y == this.y) return Double.NEGATIVE_INFINITY; // Same Point
            else return Double.POSITIVE_INFINITY; // Vertical Line
        if (that.y == this.y) return +0.0; // Horizonatal Line
    ...
    public static void main(String[] args) {
        ...
        // if (a == b) StdOut.println("Double positive equals  a b: " + (a - b));
        // if (a == c) StdOut.println("Double positive: " + Double.NEGATIVE_INFINITY);
    }
}
